...
function MetaItem({
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#FAF0E3] dark:bg-neutral-800">
        <Icon className="h-5 w-5 text-[#1D3E35] dark:text-stone-300" strokeWidth={1.8} />
      </div>
      <div className="min-w-0 pt-0.5">
        <p className="text-xs font-normal text-neutral-500 dark:text-neutral-400">{label}</p>
        <p className="mt-0.5 text-sm font-normal text-black sm:text-[15px] dark:text-stone-100">{value}</p>
      </div>
    </div>
  );
}
...
export default function ProjectAbout({ project }: ProjectAboutProps) {
  ...
  return (
    <div className="min-w-0">
        ...
        <div className="mt-12 min-w-0 overflow-hidden border-t border-neutral-200 pt-10 dark:border-neutral-800">
          <h2 className="mb-4 text-base font-normal tracking-tight text-black sm:text-lg dark:text-stone-100">Description</h2>
          <div
            className={`${discoverBody} max-w-3xl min-w-0 space-y-3 text-[13px] font-light leading-[1.6] text-black antialiased sm:text-sm dark:text-neutral-400 [&_p]:font-light [&_p]:text-black dark:[&_p]:text-neutral-400`}
          >
            {paragraphs.map((paragraph, index) => (
              <p
                key={index}
                className="font-light text-black whitespace-pre-wrap break-words [overflow-wrap:anywhere] dark:text-neutral-400"
              >
                {paragraph}
              </p>
            ))}
