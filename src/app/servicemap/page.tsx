...
import ServiceMapFilterBar from '@/components/services/ServiceMapFilterBar';
import ServiceMapCard from '@/components/services/ServiceMapCard';
import ServiceDetails from '@/components/services/ServiceDetails';
import ServiceMapPreview from '@/components/services/ServiceMapPreview';
...
import { resolveServiceMapMarkerCategoryLabel } from '@/lib/mapMarkerPrice';
...
function resolveSeller(task: Task) {
  const service = mapTaskToPublicService(task);
  return {
    name: service.author.name,
    avatar: service.author.avatar,
    verified: false,
  };
}

function isOpenService(task: Task): boolean {
  return task.status !== 'completed' && task.status !== 'cancelled';
}

function isServiceListing(task: Task): boolean {
  return getListingKind(task) === 'service';
}

function ServicemapPageContent() {
  const searchParams = useSearchParams();
  const [focusedTaskId, setFocusedTaskId] = useState<string | null>(null);
  const [detailServiceSlug, setDetailServiceSlug] = useState<string | null>(null);
  ...
  const [filters, setFilters] = useState<ServiceMapFilters>(() => ({
    ...DEFAULT_SERVICE_MAP_FILTERS,
  ...
  const loadServices = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await serviceService.getServices({ page_size: 200 });
      if (response.success && response.data) {
        setServiceTasks(extractTaskList(response.data).filter(isServiceListing).filter(isOpenService));
        return;
      }

      const result = await searchBrowseServiceTasks({
        page: 1,
        page_size: 200,
        sort_by: 'newest',
      });
      setServiceTasks(result.items.filter(isServiceListing).filter(isOpenService));
    } catch {
      try {
        const result = await searchBrowseServiceTasks({
          page: 1,
  ...
  const handleFilterChange = useCallback((next: ServiceMapFilters) => {
    setFilters(next);
  }, []);

  const filteredServiceTasks = useMemo(
    () => applyServiceMapFilters(serviceTasks, filters),
    [serviceTasks, filters],
  );

  const serviceByKey = useMemo(() => {
    const map = new Map<string, Service>();
    for (const task of filteredServiceTasks) {
      const service = mapTaskToPublicService(task);
  ...
  const browseTasksOrderKey = useMemo(
    () =>
      `${filters.sortBy ?? 'newest'}|${filteredServiceTasks
        .map((t) => String(t.id))
        .join(',')}`,
    [filteredServiceTasks, filters.sortBy],
  );
  ...
  const findTaskByKey = useCallback(
    (taskKey: string | null) => {
      ...
      return (
        filteredServiceTasks.find((t) => t.id === taskKey || t.slug === taskKey) ||
        serviceTasks.find((t) => t.id === taskKey || t.slug === taskKey) ||
        null
      );
    },
    [filteredServiceTasks, serviceTasks],
  );

  const handleViewService = useCallback((taskKey: string) => {
    const task = findTaskByKey(taskKey);
    const slug = task?.slug || taskKey;
    setFocusedTaskId(taskKey);
    setDetailServiceSlug(String(slug));
    setSheetSnap('map');
  }, [findTaskByKey]);

  const handleTaskFocus = useCallback((taskKey: string) => {
    setFocusedTaskId(taskKey);
    setDetailServiceSlug(null);
    setSheetSnap('map');
  }, []);
  ...
  const transformTasksForMap = (apiTasks: Task[]): MapTask[] =>
    apiTasks
      ...
      .map(({ task, browseOrder }) => {
        const seller = resolveSeller(task);
        const categoryName =
        ...
        return {
          ...
          title: task.title || 'Untitled Service',
          ...
          priceLabel: resolveServiceMapMarkerCategoryLabel(task),
          browseOrder,
          ...
          user: {
            ...
            rating: mapTaskToPublicService(task).rating || 0,
  ...
  const mappedTasks = useMemo(
    () => transformTasksForMap(filteredServiceTasks),
    [filteredServiceTasks, browseTasksOrderKey],
  );

  const previewEntry = useMemo(() => {
    if (!focusedTaskId || detailServiceSlug) return null;
    ...
    const service = serviceByKey.get(String(focusedTaskId)) ?? serviceByKey.get(String(mapTask.slug));
    if (!service) return null;
    return { service, mapTask };
  }, [focusedTaskId, detailServiceSlug, mappedTasks, serviceByKey]);

  const postServiceHref = getDashboardCreateHref('services');
      ...
      <main ref={mainRef} className="flex flex-1 overflow-hidden">
                    ...
                    {filteredServiceTasks.map((task) => {
                      ...
                      return (
                        <button
                          key={task.id}
                          type="button"
                          onClick={() => handleViewService(taskKey)}
                          className={`rounded-full border-2 transition-all hover:scale-110 ${
                            detailServiceSlug === task.slug || detailServiceSlug === taskKey
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col gap-3 pb-2">
                    {filteredServiceTasks.length > 0 ? (
                      filteredServiceTasks.map((task) => {
                        const taskKey = String(task.slug || task.id);
                        const service = serviceByKey.get(taskKey);
                        if (!service) return null;
                        ...
                        return (
                          <ServiceMapCard
                            ...
                            isActive={detailServiceSlug === task.slug || detailServiceSlug === taskKey}
                            onClick={() => handleViewService(taskKey)}
                          />
                        );
                      })
                      ...
                      <div className="flex h-full flex-col items-center justify-center p-8 text-center">
                        ...
                        <h3 className="mb-2 text-lg font-bold text-on-surface dark:text-stone-100">
                          {serviceTasks.length > 0 ? 'No services match your filters' : 'No services available'}
                        </h3>
                        <p className="mb-4 font-sans text-sm font-normal leading-5 text-on-surface-variant dark:text-neutral-400">
                          {serviceTasks.length > 0
                            ? 'Try adjusting search, category, price, or location filters.'
                            : 'There are currently no services listed. Be the first to add a service!'}
                        </p>
                        <a
                          href={postServiceHref}
        ...
        <div className="relative z-0 flex min-h-0 min-w-0 flex-1 flex-col">
          <ServiceMapFilterBar
            ...
            serviceCount={filteredServiceTasks.length}
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden bg-surface-dim">
            ...
            {previewEntry ? (
              <ServiceMapPreview
                ...
                onViewService={() =>
                  handleViewService(String(previewEntry.mapTask.slug || previewEntry.mapTask.id))
                }
              />
            ) : null}
            ...
            <TaskBrowseMobileSheet
              ...
              taskCount={filteredServiceTasks.length}
              hidden={Boolean(detailServiceSlug)}
              countLabel="services"
            >
              {isLoading ? (
                ...
                <div className="flex flex-col items-center px-2 py-10 text-center">
                  ...
                  <p className="mb-4 text-sm text-on-surface-variant dark:text-neutral-400">
                    {serviceTasks.length > 0 ? 'Try adjusting search or service filters.' : 'Be the first to add a service!'}
                  </p>
                  <a
                    href={postServiceHref}
                ...
                >
                  <div className="pointer-events-auto max-w-sm rounded-2xl border border-outline-variant bg-white p-6 text-center shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
                    ...
                    <p className="text-sm text-on-surface-variant dark:text-neutral-400">
                      These services don&apos;t have saved map coordinates yet. Browse the list below or
                      open a service for full details.
                    </p>
                  </div>
                </div>
              )}
            ...
            <AnimatePresence mode="wait">
              {detailServiceSlug ? (
                <ServiceDetails key={detailServiceSlug} serviceSlug={detailServiceSlug} onClose={() => setDetailServiceSlug(null)} />
              ) : null}
            </AnimatePresence>
          </div>
        </div>
